...
download_image() {
  ...
  info "Downloading DietPi Trixie VM image..."
  ...
  QCOW2_IMAGE="${DIETPI_IMAGE%.xz}"
  [[ -s "$QCOW2_IMAGE" ]] || die "The extracted DietPi qcow2 image was not found."
}
...
create_vm() {
  ...
  qm create "$VM_ID" \
    --name "$VM_NAME" \
    --description "LoxBerry 4 VM based on the official DietPi Trixie Proxmox x86_64 image." \
    --ostype l26 \
    --machine q35 \
    --bios seabios \
    --cpu kvm64 \
  ...
  import_output="$(qm importdisk "$VM_ID" "$QCOW2_IMAGE" "$STORAGE" 2>&1)"
  printf '%s\n' "$import_output"
...
main() {
  clear
  printf "${BOLD}LoxBerry 4 VM installer for Proxmox VE${RESET}\n"
  printf "Version %s - DietPi Trixie\n\n" "$SCRIPT_VERSION"
